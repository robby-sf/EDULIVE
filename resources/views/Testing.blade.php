...
@section('title', 'Teman Belajar - EDULIVE')

{{-- Menambahkan styling khusus jika diperlukan --}}
@push('styles')
    <style>
        /* Styling untuk menyembunyikan elemen Alpine.js sebelum inisialisasi */
        [x-cloak] {
...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="bg-gray-100 flex flex-col flex-grow" x-data="{
            ...
            <div class="flex flex-grow overflow-hidden">
                <div :class="{ 'w-full': !showChatbot, 'w-3/5': showChatbot }"
                    class="relative flex flex-col items-center justify-center p-4 transition-all duration-300">
                    ...
                    <div
                        class="relative w-full max-w-4xl aspect-video bg-gray-200 rounded-lg shadow-lg border-4 border-gray-300 overflow-hidden">
                        <video id="video" autoplay muted playsinline
                            class="absolute top-0 left-0 w-full h-full object-contain z-10" x-show="showCameraControls"
                            x-cloak></video>
                        <canvas id="output"
                            class="absolute top-0 left-0 w-full h-full object-contain z-50 !pointer-events-none"></canvas>
                        <div x-show="showStartButton" x-cloak
                            class="absolute inset-0 flex items-center justify-center text-gray-500 text-xl z-0">
                            Kamera belum aktif
                        </div>
                        <div class="absolute top-4 left-4 bg-black bg-opacity-60 text-white px-3 py-1 rounded-lg text-sm z-30"
                    ...
                    <div class="flex justify-center items-center mt-8 space-x-4">
                        ...
                        <button @click="toggleChatbot()"
                            class="bg-gray-200 text-gray-700 p-4 rounded-full shadow-md hover:bg-gray-300 transition">
                ...
                <div x-show="showChatbot"
                    class="w-2/5 flex flex-col bg-white shadow-lg border-l border-gray-200 overflow-hidden" x-cloak>
                    ...
                    <div class="flex-1 min-h-0 max-h-[calc(100vh-200px)] overflow-y-auto p-4 flex flex-col space-y-2">
                        <template x-for="message in chatMessages" :key="message.text">
                            <div :class="{
                                'bg-gray-900 text-white self-end': message
                                    .sender === 'user',
                                'bg-gray-200 text-gray-900 self-start': message
                                    .sender === 'assistant'
                            }"
